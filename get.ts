...
import { AtpBaseClient } from "./lex/index.ts";
...
function createXrpcClient(pdsUrl: string): AtpBaseClient {
  return new AtpBaseClient({ service: pdsUrl });
}

async function loadCollections(
  xrpcClient: AtpBaseClient,
  did: string,
): Promise<string[]> {
  const response = await xrpcClient.com.atproto.repo.describeRepo({
    repo: did,
  });
...
async function listRecords(
  ...
  const response = await xrpcClient.com.atproto.repo.listRecords({
    repo: did,
    collection,
    limit: 100,
  });
...
async function fetchRecord(
  ...
  const response = await xrpcClient.com.atproto.repo.getRecord({
    repo: did,
    collection,
    rkey,
  });
